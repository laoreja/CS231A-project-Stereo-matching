...
tf.app.flags.DEFINE_string('resume_ckpt_path', '', "path of ckpt for resume training")              
      ...
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (gcnet_model.TOWER_NAME, i)) as scope:
            loss = tower_loss(scope, hps, dataset)
...
#    summaries.append(tf.summary.scalar('learning_rate', lrn_rate))    
    ...
    if FLAGS.resume_ckpt_path != '':
      saver.restore(sess, FLAGS.resume_ckpt_path)
    else:
      sess.run(init)
