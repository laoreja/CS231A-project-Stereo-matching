...
class GCNet(object):
  ...
  def build_graph_to_predict(self):
    self._build_model()
...
#        tf.add_to_collection('shapes', tf.shape(x))
      ...
      with tf.variable_scope('layer_'+str(layer_idx)):
        layer_idx += 1
        input_shape = tf.shape(self._left_images)
...
#    self.debug_op_list.append(self.shapes)
  ...
  def _build_loss_op(self):
    with tf.variable_scope('loss'):
      self.divider = tf.reduce_sum(self.mask) + 1
      abs_error = tf.abs((self.gt_disparity - self.predicted_disparity) * self.mask)
      self.abs_loss = tf.reduce_sum(abs_error) / self.divider
...
#      tf.summary.scalar('abs_loss', self.abs_loss)
      ...
      tf.summary.image('mask', tf.expand_dims(self.mask, axis=3))        
      tf.summary.image('masked_gt', tf.expand_dims(self.gt_disparity * self.mask, axis=3))              
      tf.summary.image('predict', tf.expand_dims(self.predicted_disparity, axis=3))        
      tf.summary.image('masked_predict', tf.expand_dims(self.predicted_disparity * self.mask, axis=3))
#      tf.summary.scalar('total_loss', self.total_loss)
      if self.mode == 'eval':
        self.larger_than_3px = tf.reduce_sum(tf.cast(tf.greater(abs_error, 3), tf.float32)) / self.divider
        self.larger_than_5px = tf.reduce_sum(tf.cast(tf.greater(abs_error, 5), tf.float32)) / self.divider
        self.larger_than_7px = tf.reduce_sum(tf.cast(tf.greater(abs_error, 7), tf.float32)) / self.divider
#        self.larger_than_5px = tf.reduce_sum(tf.greater(tf.abs((self.gt_disparity - self.predicted_disparity) * self.mask, tf.constant([5])))) / divider
#        self.larger_than_7px = tf.reduce_sum(tf.greater(tf.abs((self.gt_disparity - self.predicted_disparity) * self.mask, tf.constant([7])))) / divider        
