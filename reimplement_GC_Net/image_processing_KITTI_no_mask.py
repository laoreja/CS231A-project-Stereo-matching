...
def inputs(dataset, batch_size=None, num_preprocess_threads=None):
  ...
  Returns:
    ...
    disparitys: 3D tensor of size [batch_size, FLAGS.half_height, FLAGS.half_width].
  """
  if not batch_size:
  ...
  with tf.device('/cpu:0'):
    left_images, right_images = batch_inputs(
...
def distorted_inputs(dataset, batch_size=None, num_preprocess_threads=None):
  ...
  # the forward inference and back-propagation.
  with tf.device('/cpu:0'):
    left_images, right_images, disparitys = batch_inputs(
        dataset, batch_size, train=True,
        num_preprocess_threads=num_preprocess_threads,
        num_readers=FLAGS.num_readers)
  return left_images, right_images, disparitys
...
def image_preprocessing(left_image, right_image, disparity, height, width):
  """Decode and preprocess one image for evaluation or training.

  Args:
    left_image, right_image: decoded image, dtype:tf.uint8
    disparity: decoded disparity, dtype:tf.float32
    train: boolean

  Returns:
    left_image, right_image, disparity:3-D float Tensors
  ...
  with tf.name_scope('image_preprocessing'):
    ...
    if disparity is not None:
      ...
      combined = tf.concat([left_image, right_image, disparity], axis=2)
    else:
...
def parse_example_proto(example_serialized, train):
  ...
  if train:
    ...
    return left_image, right_image, disparity, height, width
...
def batch_inputs(dataset, batch_size, train, num_preprocess_threads=None,
  ...
  Returns:
    left_images, right_images: Images. 4D tensor of size [batch_size, FLAGS.image_size,
                image_size, 3].
    disparitys: 3D tensor of size [batch_size, FLAGS.image_size, image_size].
  ...
  with tf.name_scope('batch_processing'):
    ...
    for thread_id in range(num_preprocess_threads):
      ...
      if train:
        left_image, right_image, disparity, height, width = parse_example_proto(
          example_serialized, train)
        left_images, right_images, disparitys = image_preprocessing(left_image, right_image, disparity, height, width)
        data.extend([list(item) for item in zip(left_images, right_images, disparitys)])

        left_images, right_images, disparitys = tf.train.batch_join(
            data,
...
#        tf.summary.image('disparitys', disparitys)
        ...
        left_image, right_image, height, width = parse_example_proto(
          example_serialized, train)
        left_images, right_images = image_preprocessing(left_image, right_image, None, height, width)
        data.extend([list(item) for item in zip(left_images, right_images)])

        left_images, right_images = tf.train.batch_join(
          data,
          batch_size=batch_size,
          capacity=2 * num_preprocess_threads * batch_size,
          shapes=[[FLAGS.half_height, FLAGS.half_width, FLAGS.depth], 
